...
class soScope_NB(soScope_Gaussian):
    ...
    def _compute_loss(self, st_data, sub_data, mode='mean'):
        ...
        # node_loss_2 = (1/self.scale) * (1/factor) * mse_loss((block_sum(norm_ep, self.sub_node)).view(sub_x_truth.shape[0], -1),
        #                                         x_input.view(sub_x_truth.shape[0], -1))
        node_loss_2 = (1 / self.scale) * mse_loss(
            (block_sum(epsilon, self.sub_node)).view(sub_x_truth.shape[0], -1),
            x_input.view(sub_x_truth.shape[0], -1))
