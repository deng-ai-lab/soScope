...
def get_spilt_dataset(root_path, test_ratio=0.1):
    team = []
    ...
    adj_path = root_path + 'sub_adj/'
    subspot_path = root_path + 'sub_spot/'

    for i in range(node_num):
        # 读入连接矩阵和分配矩阵
        subspot = np.load(subspot_path + 'subspot_' + str(i + 1) + '.npy')
        adj = np.load(adj_path + 'adj_' + str(i + 1) + '.npy')

        subspot = torch.tensor(subspot).float()
        sub_edge = torch.tensor(adj[:, :2].T).long()
        sub_edge_value = torch.tensor(adj[:, 2].T).float()

        # Add self loop
        sub_edge_i, sub_edge_value_i = add_self_loops(sub_edge, sub_edge_value)

        i = torch.tensor([i]).long()

        tuple_ = [subspot, sub_edge_i, sub_edge_value_i, i]
        # tuple_ = [subspot, sub_edge, sub_edge_value, i]
        team.append(tuple_)  # 添加子元组

    print("Sub Data Loaded")
    # 切分数据
    t_train, t_test = model_selection.train_test_split(team, test_size=test_ratio)
...
def get_bias_dataset(root_path):
    team = []
    bias_data_path = root_path + 'sub_spot_bias.npy'
    bias_label = np.load(bias_data_path)
    node_num = bias_label.shape[0]

    for i in range(node_num):
        # 读入连接矩阵和每个采样点特征
        subspot = bias_label[i, :].reshape(1, -1)
        adj = np.array([[0, 0, 1]])
    ...
    print("Sub Bias Data Loaded")
    return team

def get_spatial_dataset(root_path, mode):
    st_label_path = root_path + 'label.npy'

    if mode == '8':
        adj_path = root_path + 'adj_8.npy'
    elif mode == '4':
        adj_path = root_path + 'adj_4.npy'
    elif mode == '6':
        adj_path = root_path + 'adj_6.npy'
    else:
        raise 'mode: {mode} does not exist, you can only choose 8, 6 or 4'.format(mode=mode)
...
def get_bias_spatial_dataset(root_path, mode):
