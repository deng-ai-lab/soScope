...
class soScope_Poisson(soScope_Gaussian):
    def __init__(self, sub_node, **params):
        super(soScope_Poisson, self).__init__(**params)
    ...
    def decode(self, sub_z, subplot):
        ...
        _, sub_r = self.decoder(sub_z_, subplot)
        p_epsilon_given_z_f = Independent(Poisson(rate=sub_r), 1)
    ...
    def _compute_loss(self, st_data, sub_data, mode='mean'):
        ...
        graph_loss_2 = F.binary_cross_entropy(sub_adj_pred.view(-1), sub_edge.to_dense().view(-1))
        graph_loss = graph_loss_2.sum()
