...
from ..utils.modules import GraphEncoder, NodeDecoder, Nb_NodeDecoder, Poisson_NodeDecoder, JointNodeDecoder_Poisson_NB, JointNodeDecoder_Gaussian_NB
...
def median_normalize(count_matrix):
    ...
    adjust_factor = median_factor + 1e-6
    normalized_count = count_matrix/adjust_factor
    return torch.log(normalized_count + 1), adjust_factor


def min_max_normalize(count_matrix):
    max_, _ = torch.max(count_matrix, dim=0)
    min_, _ = torch.min(count_matrix, dim=0)
    adjust_factor = max_ - min_
    normalized_count = (count_matrix - min_)/adjust_factor
    return normalized_count, adjust_factor, min_
...
class VGAETrainer_NB(VGAETrainer):
    ...
    def _compute_loss(self, st_data):
        ...
        # make x a integer not less than 0
        # x_input, factor = median_normalize(x * self.sub_node)
        # x_input, factor, min_ = min_max_normalize(x * self.sub_node)
        x_input = (x * self.sub_node).float()
        x_all = (x * self.sub_node).long()
...
class VGAETrainer_Joint_GNB(VGAETrainer):
    def __init__(self, protein_dim, sub_node, **params):
        super(VGAETrainer_Joint_GNB, self).__init__(**params)
        self.protein_dim = protein_dim
        self.encoder_st = GraphEncoder(self.protein_dim + self.gene_dim, self.z_dim, head=self.head_num)
